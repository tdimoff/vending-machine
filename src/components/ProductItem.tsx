...
const ProductItem = ({
  ...
  const handleQuantityChange = (delta: number) => {
    const newQuantity = Math.max(0, Math.min(selectedQuantity + delta, product.stockQuantity));
      ...
      <Paper elevation={3} className={styles.productItem}>
        ...
        >
          ...
          <CardActions className={styles.productItem__actions}>
            <Box
              className={`${styles.productItem__quantityControl} ${
                isSelected
                  ? styles["productItem__quantityControl--visible"]
                  : ""
              }`}
            >
              {isSelected && (
